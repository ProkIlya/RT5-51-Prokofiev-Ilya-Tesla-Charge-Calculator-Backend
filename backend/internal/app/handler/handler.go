...
func (h *Handler) RegisterHandler(router *gin.Engine) {
	...
	public := api.Group("")
	{
		...
		// Маршрут для обновления заряда (доступен только по X-API-Key)
		public.PUT("/trips/:trip_id/update_charge", h.UpdateTripChargeAPI)
	}
	...
	protected.Use(h.RequireAuth())
	{
		...
		trips := protected.Group("/trips")
		{
			...
			//trips.PUT("/:trip_id/update_charge", h.UpdateTripChargeAPI) // Новый маршрут 22 метода
